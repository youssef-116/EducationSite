...
if(isset($_POST['type'])){
    ...
    if(!in_array($type,['computer science','math-1','mangement','economics','physics','information science','english','electronics'])){
        $errortype='please choose subject <br>';
        $error = 1;
    }
}
//type already 
$existance="SELECT * FROM `log_prof` WHERE name='$type'";
$users=mysqli_query($conn,$existance);
$rows=mysqli_num_rows($users);
if($rows>0){
    $errortype='<p class="error">Username already exists!</p> <br>';
    $error = 1;
}

//email already exists
$exist="SELECT * FROM `log_prof` WHERE email='$email'";
...
if(empty($phonenumber)){
    $errorphone='<p class="error">enter your phone</p>';
    $error= 1;
}
//email check
if(empty($email)){
...
if(empty($type)){
    $errorsub='<p class="error">Please enter your email!</p> <br>';
    $error = 1;
    ...
    if(($error==0)&&($rows==0)){
        $insert= "INSERT INTO log_prof(name,email,password,confirmpass,md5pass,phonenumber,homeaddress,age,subject) 
        VALUES ('$username','$email','$password','$cpassword','$md5pass','$phonenumber','$homeaddress','$age','$type')";
